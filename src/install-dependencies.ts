...
export const installDependencies = async (
    ...
    while (true) {
        try {
            ...
            if (!answers.retry) {
                console.log(
                    red("Could not install dependencies. Please rerun ") + bgRed(bold("mkts")) + red(" to try again")
                );
                break;
            }
        }
...
const installPackages = (rootFolder: string, packages: Array<string>, dev?: boolean): void => {
    if (dev) execSync(`cd ${rootFolder} && yarn add --dev ${packages.join(" ")}`, { stdio: "inherit" });
    else execSync(`cd ${rootFolder} && yarn add ${packages.join(" ")}`, { stdio: "inherit" });
};
